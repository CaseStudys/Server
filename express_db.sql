...
VALUES
  (
    ...
    5600000,
    'AAモータース',
    'ＲＺ　フルセグ　ＨＤＤナビ　ミュージックプレイヤー接続可　衝突被害軽減システム　ＥＴＣ　ＬＥＤヘッドランプ　フルエアロ',
    'pictures/1'
  ),
  (
    2,
  ...
  (
    ...
    1642000,
    'CC自動車工業',
    '【クルーズコントロール】高速道路もラクラク走行。アクセルを離しても一定速度で走行ができ、長距離運転時の負担を軽減！加速・減速も簡単なスイッチ操作で調整できます。\r\n【シートヒーター】寒い日に重宝するシートヒーター！エアコンより早く温まってくれるので寒がりの人も安心ですね♪エアコンの温風は乾燥するから苦手、という方にもをおすすめです。\r\n【純正アルミホイール】外観にマッチした純正アルミホイールを装着しております。各種社外アルミホイールや、スタッドレスタイヤも取扱いしております。是非ご相談ください♪',
    'pictures/3'
  ),
  (
    4,
    ...
    868000,
    'AAモータース',
    'ＴＶでも放映された!驚きのバリュー!カーバンクライト!●ロープライスカー大型店!●旧来にない高画質○ムービー動画●ＤＶＤ無料配送などでご検討可能！\r\n全国販売承ります●ネット成約時に陸送キャンペーン実施中です●車両の状態を動画配信●スマートフォン対応●ＤＶＤ配送無料で承っておりますのでお問合せ下さい。全５店舗！首都圏仕入れの４ＷＤ車なども常時多数在庫。当社にてタイヤ交換も承っております',
    'pictures/4'
  ),
  (
    5,
  ...
  (
    ...
    648000,
    '北島自動車工業',
    'お支払い方法のご相談もお気軽して下さい。一括現金、ローン、またはリースのご案内も可能です。法人様には節税のお話もさせて頂きます。２人乗り仕様なので、荷台は広々使えます！日産純正ラジオデッキ　ＡＭ／ＦＭ対応。新品ナビもご提案できます！',
    'pictures/6'
  );
-- --------------------------------------------------------
  --
...
--
  ...
  --
INSERT INTO
  `employees` (`employee_id`, `name`, `phone_number`)
VALUES
  (1, '社員１', '08011111111'),
  (2, '社員２', '08022222222'),
  (3, '社員３', '08033333333');
...
ADD
  PRIMARY KEY (`car_id`),
ADD
  KEY `employees_to_cars` (`employee_id`);
--
...
ADD
  KEY `users_to_projects` (`buyer_id`),
ADD
  KEY `exhibits_to_projects` (`exhibit_id`);
--
  -- テーブルのインデックス `users`
  --
ALTER TABLE
...
ALTER TABLE
  `cars`
ADD
  CONSTRAINT `employees_to_cars` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`employee_id`);
--
  -- テーブルの制約 `exhibits`
  --
ALTER TABLE
  `exhibits`
ADD
  CONSTRAINT `cars_to_exhibits` FOREIGN KEY (`car_id`) REFERENCES `cars` (`car_id`);
--
  -- テーブルの制約 `projects`
  --
ALTER TABLE
  `projects`
ADD
  CONSTRAINT `exhibits_to_projects` FOREIGN KEY (`exhibit_id`) REFERENCES `exhibits` (`exhibit_id`),
ADD
  CONSTRAINT `users_to_projects` FOREIGN KEY (`buyer_id`) REFERENCES `users` (`user_id`);
COMMIT;
