...
router.route(EXHIBIT_END_POINT).post(async (req, res) => {
  ...
  const selectExhibits = "SELECT start_date FROM exhibits where start_date LIKE '%"+startDate+"%' ORDER BY start_date desc;";
  //車両登録
  var insertExhibit = "";
  //まだ出品登録されていない車両情報を取得
  const selectCars ="SELECT c.car_id,c.picture_path,c.type_name,c.purchace_price FROM cars c LEFT JOIN exhibits e ON c.car_id = e.car_id WHERE e.exhibit_id IS NULL;";

  //DB処理
  db.mysql_connection.connect((err) => {//日時設定ーーーーーーーーーー
    db.mysql_connection.query(
      ...
      (err, result) => {
        ...
        //その日程の１台目の場合--
        if (result.length == 0) {
          insertExhibit = "INSERT INTO exhibits(car_id,start_date) VALUE("+carId+",'"+startDate+" 09:00:00');";
        }
        else {
          //２台目以降の場合--
          ...
          insertExhibit = "INSERT INTO exhibits(car_id,start_date) VALUE("+carId+",'"+insertDate+"');";
        }
        db.mysql_connection.query(//出品登録ーーーーーーーーーーーーー
          insertExhibit,
          (err, result) => {
            if (err) {
              return res.status(500).json({ code: 500, message: err });
            }
            db.mysql_connection.query(//出品登録ーーーーーーーーーーー
              selectCars,
              (err, result) => {
                if (err) {
                  return res.status(500).json({ code: 500, message: err });
