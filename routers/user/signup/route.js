...
router.route(USERSIGNUP_END_POINT).post(async (req, res) => {
    ...
    //sql文生成---
    //ユーザー登録
    const insertUser = "INSERT INTO users(distinction,name,birthday,address,phone_number,password,trust_flg) VALUES(?,?,?,?,?,?,?);";
    //出品中の車両取得
    ...
    db.mysql_connection.connect((err) => {
        ...
        (err, result) => {
            ...
            //cookie設定
            res.cookie('user_id', result[0].insertId, {maxAge:limitTime, httpOnly:false});
            res.cookie('name', body.name, {maxAge:limitTime, httpOnly:false});
            res.cookie('password', body.password, {maxAge:limitTime, httpOnly:false});
            //render
